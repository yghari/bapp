...
function renderStudents() {
    ...
    tbody.innerHTML = filtered.map((s, idx) => `<tr>
        <td>${idx+1}</div><td><strong>${escapeHtml(s.name)}</strong></div><td><span class="class-chip">${escapeHtml(s.class)}</span></div>
...
function renderBooks() {
    ...
    if (filtered.length === 0) { tbody.innerHTML = '<tr><td colspan="8" class="empty-state">Aucun livre</div></tr></tr>'; return; }
    tbody.innerHTML = filtered.map((b, idx) => `<tr><td>${idx+1}</div><td><strong>${escapeHtml(b.title)}</strong></div><td><span class="class-chip">${escapeHtml(b.class)}</span></div>
...
function renderClasses() {
    ...
    if (getClasses().length === 0) { tbody.innerHTML = '<tr><td colspan="6" class="empty-state">Aucune classe</div></td></tr>'; return; }
    tbody.innerHTML = getClasses().map((c, idx) => `<tr><td>${idx+1}</div><td><strong>${escapeHtml(c.name)}</strong></div><td>${escapeHtml(c.level)}</div>
...
function renderStock() {
    ...
    if (getOrders().length === 0) { ordersTbody.innerHTML = '<tr><td colspan="7" class="empty-state">Aucune commande</div></td></tr>'; }
    else { ordersTbody.innerHTML = getOrders().map((o, idx) => `<tr><td>${idx+1}</div><td>${escapeHtml(o.bookTitle)}</div><td><span class="class-chip">${escapeHtml(o.class)}</span></div>
    ...
    else { historyTbody.innerHTML = recent.map((h, idx) => `<tr><td>${idx+1}</div><td>${h.date}</div><td>${escapeHtml(h.bookTitle)}</div><td>${h.action}</div>
...
function renderDashboard() {
    ...
    container.innerHTML = `
        ...
        <div id="panel-classes" class="panel"><div class="toolbar"><button id="addClassBtn" class="primary">+ Ajouter Classe</button></div><div class="table-wrap"><table><thead><tr><th>#</th><th>Classe</th><th>Niveau</th><th>Élèves</th><th>Livres</th><th>Actions</th></tr></thead><tbody id="classesTbody"></tbody></table></div></div>
